...
//usFinalElapsed = 21599;
	...
	TraceInternal(_D("m_pLicenseData->nAmberLimit = %d"), m_pLicenseData->nAmberLimit);
	TraceInternal(_D("m_pLicenseData->nRedLimit   = %d"), m_pLicenseData->nRedLimit);
	...
	for (int i = 0; i < ARRAYSIZE(m_fnClients); i++)
	{
		...
		if (m_fnClients[i] != NULL)
		{
			...
			} else if ((short)usFinalRemaining < m_pLicenseData->nAmberLimit)
			{
				if ((short)usFinalRemaining < m_pLicenseData->nRedLimit)
				{
...
// {BE340003-4E1D-0D74-4459-345036C74128}
	...
	for (int j = 0, i = WIV_MAX_ENCRYPTED_LICENSE_LENGTH/2 ; j < WIV_MAX_ENCRYPTED_LICENSE_LENGTH ; )
	{
		BYTE s1 = 0;
		BYTE s2 = 0;
								  ...
								  const USHORT usRemaining, const DWORD dwSession)
{
	...
	for (int i = WIV_MAX_ENCRYPTED_LICENSE_LENGTH/2; i < (int)sizeof(bWork); i++)
	{
		bWork[i++] ^= b2a;
		bWork[i] ^= b2b;
	}
...
int CWiVLicense::StorageDiskRead(const int nDsk, const int nStartByte, const int nNumBytes, BYTE* pbBuffer)
{
    ...
    if (hDsk == NULL)
	{
		...
		TraceError(_D("CWiVLicense::StorageDiskRead: StorageDiskOpen failed, nDsk = %d, hrResult = 0x%08X"), nDsk, hrResult);
    ...
    {
        hrResult= GetLastError();
		TraceError(_D("CWiVLicense::StorageDiskRead: StorageDiskGetInfo failed, hrResult = 0x%08X"), hrResult);
        CloseHandle(hDsk);
		hDsk = NULL;
		TraceLeave(_D("CWiVLicense::StorageDiskRead"), hrResult, tlInternal);
        return hrResult;
	...
	TraceInternal(_D("CWiVLicense::StorageDiskRead: nDsk = %d, nNumBytes = %d, pbBuffer = <%s>"), nDsk, nNumBytes, BtoS(pbBuffer, nNumBytes));
