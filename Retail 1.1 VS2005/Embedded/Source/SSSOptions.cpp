...
CSSSOptions::CSSSOptions(HMODULE hmModule, CSSSPhone *pPhone, bool bFullScreen)
{
	...
	if(_tcslen(g_szNoLicenseLabel) <= 0)
	{
		if (!g_pSSSData)
		{
			...
			_snwprintf(szTitle, MAX_PATH, _T("%s %s"), _T("SIM Status Switcher"), SSS_TEXT_OPTIONS);
		}
		else
		{
			_snwprintf(szTitle, MAX_PATH, _T("%s %s"), _tcsrev(g_pSSSData->szRevProductName), SSS_TEXT_OPTIONS);
		}
	}
	else
	{
		_snwprintf(szTitle, MAX_PATH, _T("%s %s"), g_szProductName, SSS_TEXT_OPTIONS);
	}
...
DWORD CSSSOptions::GetSecurityOptions(bool bForce)
{
	...
	if (_tcslen(szName) != 0)
	{
		...
		dwResult = RegGetValue(hKey, szName, m_abPIN, WIV_MAX_BINARY);//, bPIN1);
...
//	memset (ppsp, 0, sizeof (psp));
	...
	psh.pszCaption = SSS_TEXT_OPTIONS;
	psh.nPages = nPages;
	psh.nStartPage = 0;
	psh.ppsp = ppsp;
...
BOOL CSSSOptions::OnInitDialog(HWND hWnd, HBRUSH *haBrush)
{
	...
	{
		haBrush[0] = GetBkBrush( hWnd, IDC_SECURITY_STATIC_HEADER1, m_hbmpBackgroundBitmap );
		haBrush[1] = GetBkBrush( hWnd, IDC_SECURITY_STATIC_HEADER2, m_hbmpBackgroundBitmap );
		haBrush[2] = GetBkBrush( hWnd, IDC_SECURITY_STATIC_AUTO_PIN, m_hbmpBackgroundBitmap );
		haBrush[3] = GetBkBrush( hWnd, IDC_SECURITY_CHECK_ALLOW_AUTO_PIN_INIT, m_hbmpBackgroundBitmap );
		haBrush[4] = GetBkBrush( hWnd, IDC_SECURITY_CHECK_ALLOW_AUTO_PIN_RADIOON, m_hbmpBackgroundBitmap );
		haBrush[5] = GetBkBrush( hWnd, IDC_SECURITY_BUTTON_CHANGE_PIN, m_hbmpBackgroundBitmap );
		haBrush[6] = GetBkBrush( hWnd, IDC_SECURITY_STATIC_ERROR, m_hbmpBackgroundBitmap );
		haBrush[7] = GetBkBrush( hWnd, IDC_SECURITY_STATIC_ENTER_PIN, m_hbmpBackgroundBitmap );
		haBrush[8] = GetBkBrush( hWnd, IDC_SECURITY_EDIT_ENTER_PIN, m_hbmpBackgroundBitmap );
		haBrush[9] = GetBkBrush( hWnd, IDC_SECURITY_BUTTON_ENTER_PIN_OK, m_hbmpBackgroundBitmap );
		haBrush[10] = GetBkBrush( hWnd, IDC_SECURITY_BUTTON_ENTER_PIN_CANCEL, m_hbmpBackgroundBitmap );
...
LRESULT CSSSOptions::OnCtlColor(HBRUSH *haBrush, HWND hWndCtrl)
{
	...
	}else if (haBrush == m_ahbrushSecurity)
	{
		switch( nID )
		{
		case IDC_SECURITY_STATIC_HEADER1:
			{
				lrResult = (LRESULT)haBrush[0];
				break;
			}
		case IDC_SECURITY_STATIC_HEADER2:
			{
				lrResult = (LRESULT)haBrush[1];
				break;
			}
		...
		case IDC_SECURITY_STATIC_ERROR:
			{
				lrResult = (LRESULT)haBrush[6];
				break;
			}
		case IDC_SECURITY_STATIC_ENTER_PIN:
			{
				lrResult = (LRESULT)haBrush[7];
				break;
			}
		case IDC_SECURITY_EDIT_ENTER_PIN:
			{
				lrResult = (LRESULT)haBrush[8];
				break;
			}
		case IDC_SECURITY_BUTTON_ENTER_PIN_OK:
			{
				lrResult = (LRESULT)haBrush[9];
				break;
			}
		case IDC_SECURITY_BUTTON_ENTER_PIN_CANCEL:
			{
				lrResult = (LRESULT)haBrush[10];
				break;
			}
...
void CSSSOptions::DrawSecurityHeader(HWND hWnd)
{
	...
	GetDrawRect(hWnd, IDC_SECURITY_STATIC_HEADER1, szTemp, &rectDraw);
...
void CSSSOptions::DrawSecurityError(HWND hWnd, LPCTSTR lpszError)
{
	...
	if (IsSecondEdition() && InWideMode())
	{
		DrawStaticText(hWnd, IDC_SECURITY_STATIC_ERROR, lpszError, DT_VCENTER | DT_CENTER, FW_NORMAL, WIV_FONT_MEDIUM, g_crRedError);
	}
	else
	{
		DrawStaticText(hWnd, IDC_SECURITY_STATIC_ERROR, lpszError, DT_VCENTER | DT_CENTER, FW_NORMAL, WIV_FONT_NONE, g_crRedError);
	}
...
void CSSSOptions::ClearSecurityError(HWND hWnd)
{
	...
	ClearStaticText(hWnd, IDC_SECURITY_STATIC_ERROR, (NULL == m_hbmpBackgroundBitmap ? GetSysColorBrush(COLOR_WINDOW) : m_ahbrushSecurity[6]));
...
LPCTSTR CSSSOptions::SetupSecurityPage(HWND hWnd, bool blFromPaint)
{
	...
	TraceEnter(_D("CSSSOptions::SetupSecurityPage"));
	...
	ShowWindow(ItemHandleFromID(hWnd, IDC_SECURITY_EDIT_ENTER_PIN), bAuto & bEnterPIN); 
	ShowWindow(ItemHandleFromID(hWnd, IDC_SECURITY_BUTTON_ENTER_PIN_OK), bAuto & bEnterPIN); 
	ShowWindow(ItemHandleFromID(hWnd, IDC_SECURITY_BUTTON_ENTER_PIN_CANCEL), bAuto & bEnterPIN);
	if (bAuto & !bEnterPIN)
	{
		TraceDetail(_D("CSSSOptions::SetupSecurityPage: Clearing IDC_SECURITY_STATIC_ENTER_PIN"));
		ClearStaticText(hWnd, IDC_SECURITY_STATIC_ENTER_PIN, (NULL == m_hbmpBackgroundBitmap ? GetSysColorBrush(COLOR_WINDOW) : m_ahbrushSecurity[7]));

		ShowWindow(ItemHandleFromID(hWnd, IDC_SECURITY_BUTTON_CHANGE_PIN), true); 
	}
	else
	{
		ShowWindow(ItemHandleFromID(hWnd, IDC_SECURITY_BUTTON_CHANGE_PIN), false); 
	}

	// Enable/Disable other controls
	EnableWindow(ItemHandleFromID(hWnd, IDC_SECURITY_CHECK_ALLOW_AUTO_PIN_INIT), !bEnterPIN); 
	EnableWindow(ItemHandleFromID(hWnd, IDC_SECURITY_CHECK_ALLOW_AUTO_PIN_RADIOON), !bEnterPIN); 
	EnableWindow(ItemHandleFromID(hWnd, IDC_SECURITY_BUTTON_ENTER_PIN_OK), bAuto & bEnterPIN); 
	EnableWindow(ItemHandleFromID(hWnd, IDC_SECURITY_BUTTON_ENTER_PIN_CANCEL), bAuto); 

	if (bAuto & bEnterPIN & !bPINExists & !bCurrentPIN & !bNewPIN & !bConfirmPIN)
	{
		TraceDetail(_D("CSSSOptions::SetupSecurityPage: Drawing SSS_TEXT_SECURITY_STATIC_ENTER_PIN"));
		lpszText = SSS_TEXT_SECURITY_STATIC_ENTER_PIN;
	}
	if (bAuto & bEnterPIN & bCurrentPIN)
	{
		TraceDetail(_D("CSSSOptions::SetupSecurityPage: Drawing SSS_TEXT_SECURITY_STATIC_CURRENT"));
		lpszText = SSS_TEXT_SECURITY_STATIC_CURRENT;
	}
	if (bAuto & bEnterPIN & bNewPIN)
	{
		TraceDetail(_D("CSSSOptions::SetupSecurityPage: Drawing SSS_TEXT_SECURITY_STATIC_NEW"));
		lpszText = SSS_TEXT_SECURITY_STATIC_NEW;
	}
	if (bAuto & bEnterPIN & bConfirmPIN)
	{
		TraceDetail(_D("CSSSOptions::SetupSecurityPage: Drawing SSS_TEXT_SECURITY_STATIC_CONFIRM"));
		lpszText = SSS_TEXT_SECURITY_STATIC_CONFIRM;
	}
	...
	{
		TraceDetail(_D("CSSSOptions::SetupSecurityPage: Drawing text"));
		_snwprintf(szInfo, WIV_MAX_STRING, _T("%s:"), lpszText);

		if (IsSecondEdition() && InWideMode())
		{
			DrawStaticText(hWnd, IDC_SECURITY_STATIC_ENTER_PIN, szInfo, DT_RIGHT, FW_NORMAL, WIV_FONT_MEDIUM);
		}
		else
		{
			DrawStaticText(hWnd, IDC_SECURITY_STATIC_ENTER_PIN, szInfo, DT_RIGHT);
		}
	}

	if (bEnterPIN) SetFocus(ItemHandleFromID(hWnd, IDC_SECURITY_EDIT_ENTER_PIN));

	TraceDetail(_D("CSSSOptions::SetupSecurityPage: dwCurStep   = %d"), dwCurStep);
	TraceDetail(_D("CSSSOptions::SetupSecurityPage: bAuto       = %d"), bAuto);
	TraceDetail(_D("CSSSOptions::SetupSecurityPage: bCurrentPIN = %d"), bCurrentPIN);
	TraceDetail(_D("CSSSOptions::SetupSecurityPage: bNewPIN     = %d"), bNewPIN);
	TraceDetail(_D("CSSSOptions::SetupSecurityPage: bConfirmPIN = %d"), bConfirmPIN);
	TraceDetail(_D("CSSSOptions::SetupSecurityPage: bEnterPIN   = %d"), bEnterPIN);
	TraceDetail(_D("CSSSOptions::SetupSecurityPage: bPINExists  = %d"), bPINExists);

	TraceLeave(_D("CSSSOptions::SetupSecurityPage"), (DWORD)lpszText);
	...
	case PSCB_GETLINKTEXT:
		{
			...
			_snwprintf((LPTSTR)lParam, WIV_MAX_STRING, _T("%s <%s,20{%s}>."), SSS_TEXT_OPTIONS_VIEWORCHANGE, g_szLinkText, SSS_TEXT_PHONE_SETTINGS);
			TraceLeave(_D("PropSheetProc"), (DWORD)0);
			return 0;
		}
	case PSCB_PRECREATE :
	...
	case PSM_QUERYSIBLINGS :
		{
			...
			NewSettings.dwTapAction = SendDlgItemMessage (hWnd, IDC_ACTIONS_COMBO_TAP_ACTION, CB_GETCURSEL, 0, 0);
			NewSettings.dwTAHAction = SendDlgItemMessage (hWnd, IDC_ACTIONS_COMBO_TAH_ACTION, CB_GETCURSEL, 0, 0);
			NewSettings.dwTodayIconTapAction = SendDlgItemMessage (hWnd, IDC_ACTIONS_COMBO_TODAY_ICON_TAP_ACTION, CB_GETCURSEL, 0, 0);
			NewSettings.dwTodayIconTAHAction = SendDlgItemMessage (hWnd, IDC_ACTIONS_COMBO_TODAY_ICON_TAH_ACTION, CB_GETCURSEL, 0, 0);
			TraceLeave(_D("CSSSOptions::DlgProcActions"), (DWORD)0);
			return 0;
		}
	case WM_NOTIFY :
		{
			...
			else if (uiCode == PSN_APPLY)
			{
				...
				// Get current selection for each action list.
				m_dwTapAction = SendDlgItemMessage (hWnd, IDC_ACTIONS_COMBO_TAP_ACTION, CB_GETCURSEL, 0, 0);
				if (m_dwTapAction == CB_ERR)
					m_dwTapAction = 0;
				m_dwTAHAction = SendDlgItemMessage (hWnd, IDC_ACTIONS_COMBO_TAH_ACTION, CB_GETCURSEL, 0, 0);
				if (m_dwTAHAction == CB_ERR)
					m_dwTAHAction = 0;
				m_dwTodayIconTapAction = SendDlgItemMessage (hWnd, IDC_ACTIONS_COMBO_TODAY_ICON_TAP_ACTION, CB_GETCURSEL, 0, 0);
				if (m_dwTodayIconTapAction == CB_ERR)
					m_dwTodayIconTapAction = 0;
				m_dwTodayIconTAHAction = SendDlgItemMessage (hWnd, IDC_ACTIONS_COMBO_TODAY_ICON_TAH_ACTION, CB_GETCURSEL, 0, 0);
				if (m_dwTodayIconTAHAction == CB_ERR)
					m_dwTodayIconTAHAction = 0;
							...
							if (nIndex >= 0)
							{
								...
								if(_tcslen(g_szNoLicenseLabel) <= 0)
								{
									if (!g_pSSSData)
									{
										...
										_snwprintf(szTitle, MAX_PATH, _T("%s %s"), _T("WiVSSS"), SSS_TEXT_OPTIONS);
									}
									else
									{
										_snwprintf(szTitle, MAX_PATH, _T("%s %s"), _tcsrev(g_pSSSData->szRevProductName), SSS_TEXT_OPTIONS);
	...
	case WM_INITDIALOG :
		{
			...
			SendDlgItemMessage (hWnd, IDC_SECURITY_BUTTON_ENTER_PIN_OK, WM_SETFONT, (WPARAM)m_hfSemiBoldFont, (LPARAM)TRUE);
			SendDlgItemMessage (hWnd, IDC_SECURITY_BUTTON_ENTER_PIN_CANCEL, WM_SETFONT, (WPARAM)m_hfSemiBoldFont, (LPARAM)TRUE);
			...
			SendDlgItemMessage (hWnd, IDC_SECURITY_EDIT_ENTER_PIN, EM_LIMITTEXT, SSS_MAX_PIN, 0);

			// Setup up appropriate sequence for PIN entry
			if (m_blAllowAutoPINAfterInit | m_blAllowAutoPINAfterRadioON)
			{
				SendMessage(hWnd, DM_SETDEFID, IDC_SECURITY_BUTTON_CHANGE_PIN, 0);
					...
					case BN_CLICKED:
						{
							...
							SendMessage(hWnd, DM_SETDEFID, IDC_SECURITY_BUTTON_ENTER_PIN_OK, 0);
							m_dwSecuritySeqStep++; 
					...
					case BN_CLICKED:
						{
							...
							TraceDetail(_D("CSSSOptions::DlgProcSecurity: BN_CLICKED received for Security: IDC_SECURITY_BUTTON_ENTER_PIN_OK"));

							ClearSecurityError(hWnd);

							SendDlgItemMessage (hWnd, IDC_SECURITY_EDIT_ENTER_PIN, WM_GETTEXT,
												  sizeof(szPIN), (LPARAM)szPIN);

							if (GetCurrentSeqStep() == g_dwSecuritySequenceActionConfirm)
							{
								TraceDetail(_D("CSSSOptions::DlgProcSecurity: Confirm PIN"));
								if (_tcsncmp(m_szEnteredPIN, szPIN, SSS_MAX_PIN) == 0)
								{
									_tcsncpy(m_szPIN, m_szEnteredPIN, SSS_MAX_PIN);
									_zclr(m_szEnteredPIN);

									SendDlgItemMessage (hWnd, IDC_SECURITY_EDIT_ENTER_PIN, WM_SETTEXT,
								...
								else
								{
									...
									SendDlgItemMessage (hWnd, IDC_SECURITY_EDIT_ENTER_PIN, EM_SETSEL, 0, -1);
								}
							}
							else if (GetCurrentSeqStep() == g_dwSecuritySequenceActionCurrent)
							{
								TraceDetail(_D("CSSSOptions::DlgProcSecurity: Current PIN"));
							...
							{
								TraceDetail(_D("CSSSOptions::DlgProcSecurity: Enter PIN"));
								_tcsncpy(m_szEnteredPIN, szPIN, SSS_MAX_PIN);
								SendDlgItemMessage (hWnd, IDC_SECURITY_EDIT_ENTER_PIN, WM_SETTEXT,
													  SSS_MAX_PIN, (LPARAM)WIV_EMPTY_STRING);
								m_dwSecuritySeqStep++;
			...
			case IDC_SECURITY_BUTTON_ENTER_PIN_CANCEL:
				{
					...
					case BN_CLICKED:
						{
							TraceInfo(_D("CSSSOptions::DlgProcSecurity: BN_CLICKED received for Security: IDC_SECURITY_BUTTON_ENTER_PIN_CANCEL"));
			...
			case IDC_SECURITY_EDIT_ENTER_PIN:
				{
					...
					case EN_CHANGE:
						{
							...
							TraceInfo(_D("CSSSOptions::DlgProcSecurity: EN_CHANGE received for Debug:IDC_SECURITY_EDIT_ENTER_PIN"));

							SendDlgItemMessage (hWnd, IDC_SECURITY_EDIT_ENTER_PIN, WM_GETTEXT,
												  sizeof(szPIN), (LPARAM)szPIN);

							EnableWindow(ItemHandleFromID(hWnd, IDC_SECURITY_BUTTON_ENTER_PIN_OK),
					...
					case EN_SETFOCUS:
						{
							...
							TraceInfo(_D("CSSSOptions::DlgProcSecurity: EN_SETFOCUS received for Debug:IDC_SECURITY_EDIT_ENTER_PIN"));
					...
					case EN_KILLFOCUS:
						{
							TraceInfo(_D("CSSSOptions::DlgProcSecurity: EN_KILLFOCUS received for Debug:IDC_SECURITY_EDIT_ENTER_PIN"));
	...
	case WM_PAINT :
		{
			...
			TraceDetail(_D("CSSSOptions::DlgProcSecurity: WM_PAINT, current step = %d"), GetCurrentSeqStep());
            ...
            SendDlgItemMessage (hWnd, IDC_SECURITY_BUTTON_ENTER_PIN_OK, WM_SETTEXT,
                                  0, (LPARAM)SSS_TEXT_OK);
            SendDlgItemMessage (hWnd, IDC_SECURITY_BUTTON_ENTER_PIN_CANCEL, WM_SETTEXT,
			...
			if (SetupSecurityPage(hWnd, true) != NULL)
			{
				TraceDetail(_D("CSSSOptions::DlgProcSecurity: Drawing IDC_SECURITY_STATIC_ENTER_PIN"));

				_snwprintf(szInfo, WIV_MAX_STRING, _T("%s:"), SetupSecurityPage(hWnd, true));
				if (IsSecondEdition() && InWideMode())
				{
					DrawStaticText(hWnd, IDC_SECURITY_STATIC_ENTER_PIN, szInfo, DT_RIGHT, FW_NORMAL, WIV_FONT_MEDIUM);
				}
			...
			{
				TraceDetail(_D("CSSSOptions::DlgProcSecurity: Clearing IDC_SECURITY_STATIC_ENTER_PIN"));
				ClearStaticText(hWnd, IDC_SECURITY_STATIC_ENTER_PIN, (NULL == m_hbmpBackgroundBitmap ? GetSysColorBrush(COLOR_WINDOW) : m_ahbrushSecurity[7]));
			}
