...
void CSSSToday::SetButtonAction(const DWORD dwAction)
{
	TraceEnter(_D("CSSSToday::SetButtonAction"));
	m_dwButtonAction = dwAction;
	TraceLeave(_D("CSSSToday::SetButtonAction"));
}
...
bool CSSSToday::SetupPhoneStateFlags()
{
	...
	if ((dwRadioSupport == RIL_RADIOSUPPORT_ON) && (dwEquipmentState == RIL_EQSTATE_FULL))
	{
		...
		Flags.DefaultSIM = TodayFlags.TodayBits.DefaultSIM;
	}
...
//		TraceDetail(_D("CSSSToday::SetupPhoneStateFlags: Back from UnlockSIM"));
	...
	if (Flags.RadioIsOn)
	{
		...
		TodayFlags.TodayBits.DefaultSIM = Flags.DefaultSIM & !Flags.TapToRegister;
	}
	else
	{
		...
		TodayFlags.TodayBits.DefaultSIM = Flags.DefaultSIM;
	}
	...
	TraceDetail(_D("CSSSToday::SetupPhoneStateFlags: DefaultSIM          = %d"), TodayFlags.TodayBits.DefaultSIM);
	TraceDetail(WIV_EMPTY_STRING);
...
LRESULT CSSSToday::ShowContextMenu (HWND hWnd, UINT uiMenuID, POINT ptTap)
{
	...
	miiMII.dwTypeData = (LPTSTR)SSS_TEXT_MENU_OPTIONS;
	SetMenuItemInfo(hmMenu, 0, true, &miiMII );	
	
	miiMII.dwTypeData = (LPTSTR)SSS_TEXT_MENU_REFRESH;
	SetMenuItemInfo(hmMenu, 2, true, &miiMII );	
	
	miiMII.dwTypeData = (LPTSTR)SSS_TEXT_MENU_SWITCH_SIM;
	SetMenuItemInfo(hmMenu, 3, true, &miiMII );	

	miiMII.dwTypeData = (LPTSTR)SSS_TEXT_MENU_PHONE_SETTINGS;
	SetMenuItemInfo(hmMenu, 6, true, &miiMII );

	miiMII.dwTypeData = (LPTSTR)SSS_TEXT_MENU_ABOUT;
	...
	if (!TodayFlags.TodayBits.RadioIsOn)
	{
		...
		miiMII.dwTypeData = (LPTSTR)SSS_TEXT_MENU_TURN_PHONE_ON;
		SetMenuItemInfo(hmMenu, 4, true, &miiMII );	
		EnableMenuItem(hmMenu, 6, MF_BYPOSITION | MF_GRAYED); 
	}
	else
	{
		miiMII.dwTypeData = (LPTSTR)SSS_TEXT_MENU_TURN_PHONE_OFF;
		SetMenuItemInfo(hmMenu, 4, true, &miiMII );	
		if (!TodayFlags.TodayBits.SIMReady)
		{
			EnableMenuItem(hmMenu, 6, MF_BYPOSITION | MF_GRAYED); 
